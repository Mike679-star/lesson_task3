...
def train(args):
    ...
    for image_path in sorted(glob.glob('{}/*'.format(args.images_dir))):
        ...
        # # 因为存储为h5文件 需要数据都为同一维度 所以将所有数据按照最小图片的标准分割
        # # 将数据分割
        # for i in range(0, hr.shape[0] - 76 + 1, 76):
        #     for j in range(0, hr.shape[1] - 76 + 1, 76):
        #         '''
        #         图像的shape是宽度、高度和通道数,shape[0]是指图像的高度=320;shape[1]是图像的宽度=480; shape[2]是指图像的通道数
        #         '''
        #         # lr_patches.append(lr[i:i + 76, j:j + 76])
        #         hr_patches.append(hr[i:i + 76, j:j + 76])
        # for i in range(0, lr.shape[0] - 19 + 1, 19):
        #     for j in range(0, lr.shape[1] - 19 + 1, 19):
        #         '''
        #         图像的shape是宽度、高度和通道数,shape[0]是指图像的高度=320;shape[1]是图像的宽度=480; shape[2]是指图像的通道数
        #         '''
        #         lr_patches.append(lr[i:i + 19, j:j + 19])
        #         # hr_patches.append(hr[i:i + 76, j:j + 76])
        # # lr_patches.append(lr)
        # # hr_patches.append(hr)
        # # print(hr.shape)
